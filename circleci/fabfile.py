...
def integ_test(repo: str = 'git@github.com:facebookincubator/magma.git',
    ...
    build_number = int(os.environ['CIRCLE_BUILD_NUM'])
    workflow_list = list(filter(lambda x: x, workflow_names.split(';')))
    circle_key_val = local(f'cat {circle_key}', capture=True)
    lease = _acquire_lease_with_retry(repo, build_number, workflow_list,
                                      circle_key_val,
    ...
    try:
        ...
        if build_package and env.stack == LTE_STACK:
            # Destroy the VM if we didn't use it to run the integ tests in
            # this same job
            should_destroy_vm = not run_integ_test
        ...
        if deploy_artifacts:
            if env.stack == LTE_STACK:
                ...
                should_rebuild = not run_integ_test
        ...
        print('Remove run timed out, this probably indicates a problem '
              'with the job')
    finally:
